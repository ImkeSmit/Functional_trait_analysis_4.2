...
##Only for the facilitation plotspecific species
library(tidyverse)
library(tidylog)
library(glmmTMB)
library(ggplot2)
...
FD_results <- read.csv("Functional trait data\\results\\FD_results_11Mar2024_for_facilitation_species.csv", row.names = 1) |> 
  filter(!nsp == "only one sp") #remove rows with only one sp
...
write.csv(results_table, "Functional trait data\\results\\FD_model_results_11Mar2024_for_facilitation_species.csv")


###Interpret model results####
mod_results <- read.csv("Functional trait data\\results\\FD_model_results_11Mar2024_for_facilitation_species.csv", row.names = 1)
