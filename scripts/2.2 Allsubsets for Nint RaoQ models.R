...
formulas <- AllSubsets(ResponseVariableColumn = which(colnames(nintc_rich_sum) == "mean_NIntc_rich_binom"), 
                       PredictorsColumns = c(which(colnames(nintc_rich_sum) %in% c("RaoQ", "graz", "aridity"))), 
                       data.source = nintc_rich_sum, 
                       Add.PolynomialTerms = TRUE,
                       Polynom.exclude = c(which(colnames(nintc_rich_sum) %in% c("RaoQ", "graz"))), 
                       Polynom.order = 2, 
...
write.csv(formula_table, "Functional trait data\\Results\\model_formulas_with_RaoQ.csv")
